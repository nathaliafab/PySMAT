...
from nimrod.core.merge_scenario_under_analysis import ScenarioInformation as ScenarioFiles
...
class TestSuitesExecution:
    def __init__(self, test_suite_executor: PythonTestSuiteExecutor) -> None:
        self._test_suite_executor = test_suite_executor

    def execute_test_suites(self, test_suites: List[TestSuite], scenario_files: ScenarioFiles) -> List[TestCaseExecutionInMergeScenario]:
        ...
        for test_suite in test_suites:
            ...
            results_base = self._test_suite_executor.execute_test_suite(test_suite, scenario_files.base, branch="base")
            logging.info("Starting execution of %s test suite in left", test_suite.generator_name)
            results_left = self._test_suite_executor.execute_test_suite(test_suite, scenario_files.left, branch="left")
            logging.info("Starting execution of %s test suite in right", test_suite.generator_name)
            results_right = self._test_suite_executor.execute_test_suite(test_suite, scenario_files.right, branch="right")
            logging.info("Starting execution of %s test suite in merge", test_suite.generator_name)
            results_merge = self._test_suite_executor.execute_test_suite(test_suite, scenario_files.merge, branch="merge")
    ...
    def execute_test_suite_with_coverage(self, test_suite: TestSuite, target_file: str, test_cases: List[str]) -> str:
        return self._test_suite_executor.execute_test_suite_with_coverage(test_suite, target_file, test_cases)
