...
### python_executable
SMAT will use the system Python by default. You can override this by setting the `python_executable` property in the configuration file to point to a specific Python interpreter.
...
This property consists of an array with the name of the Generators which will be used in SMAT during Test Suite Generation step. If not set, all the implemented generators will be used. Valid values are: `gemini` and `project`.
